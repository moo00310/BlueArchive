...
### 9. 멀티플레이어 로그인 및 서버 데이터 동기화
- 닉네임 → GUID 기반 UID 매핑 (동일 닉네임은 세션·기기 무관 동일 데이터)
- Server RPC 기반 로그인 플로우 (PIE 환경의 URL 옵션 미전달 이슈 해결)
- 서버 권위 설계: 재화·캐릭터 데이터는 서버 SaveGame으로 관리
- 클라이언트 로컬 세이브(파티 프리셋)를 닉네임별로 분리하여 계정 전환 지원
- GameMode PreLogin / PostLogin / RPC를 활용한 접속 생명주기 관리
